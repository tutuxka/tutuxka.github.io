...
$.ajax({
  type: 'get',
  url: 'https://china.tutuxka.eu.org/api/ip',
...
function showWelcome() {
  ...
  let dist = getDistance(114.442627,23.090711, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function getWeibo() {
  fetch('https://weibo.tutuxka.eu.org/api').then(data => data.json()).then(data => {  // 这里要写上你的API!!!
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by 兔兔博客🇨🇳 %c 你正在访问兔兔博客🇨🇳の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 兔兔博客🇨🇳") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('兔兔博客🇨🇳的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if ((lunar["IMonthCn"] == "腊月" && lunar["IDayCn"] == "廿六")) {
  //站长生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
...
//     $('#newYear').wpSuperSnow({
//       flakes: ['https://photo.tutuxka.eu.org/yb1.webp', 'https://photo.tutuxka.eu.org/yb2.webp', 'https://photo.tutuxka.eu.org/yb3.webp'],
//       totalFlakes: '100',
//       zIndex: '999999',
//       maxSize: '30',
//       maxDuration: '20',
...
function createtime() {
  ...
  var start = new Date("02/09/2022 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("02/09/2022 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://photo.tutuxka.eu.org/tutu.svg' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://photo.tutuxka.eu.org/小屋-下班休息啦.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("font") == undefined) {
  localStorage.setItem("font", "BlogFont");
}
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(/assets/tutu.jpg);
    --darkmode-bg:url(/assets/tutu.jpg);
    --mobileday-bg: url(/assets/tutu.jpg);
    --mobilenight-bg: url(/assets/tutu.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
...
<details class="folding-tag" cyan><summary> 设置自定义背景 </summary>
              <div class='content'>
              <p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://xxx.com/xxx.webp"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🌈切换背景🌈</button></center></p>
              </div>
            </details>
